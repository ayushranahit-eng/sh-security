...
EOF
  ...
  extract_dir="$CACHE_DIR/gitleaks-extract.$$"
  ...
  if ! download_file "$url" "$archive"; then
    log "Could not download gitleaks; continuing without gitleaks"
    return 1
  fi
  rm -rf -- "$extract_dir"
  mkdir -p "$extract_dir"
  if ! tar -xzf "$archive" -C "$extract_dir" >/dev/null 2>&1; then
    log "Could not extract gitleaks archive; continuing without gitleaks"
    rm -rf -- "$extract_dir"
    return 1
  fi
  found="$(find "$extract_dir" -type f -name gitleaks -perm -u+x 2>/dev/null | head -n 1)"
  if [ -z "$found" ]; then
    found="$(find "$extract_dir" -type f -name gitleaks 2>/dev/null | head -n 1)"
  fi
  if [ -z "$found" ]; then
    log "gitleaks binary was not found in the downloaded archive; continuing without gitleaks"
    rm -rf -- "$extract_dir"
    return 1
  fi
  mv "$found" "$tool" || { rm -rf -- "$extract_dir"; return 1; }
  chmod +x "$tool" || { rm -rf -- "$extract_dir"; return 1; }
  rm -rf -- "$extract_dir"
  printf '%s\n' "$tool"
}
